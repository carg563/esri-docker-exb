 ...
 */
   ...
   *  - `records` of selection view are updated automatically whenever `sourceRecords` or filter changes,
   ...
   *     - if load data, `sourceRecords` and `records` may be added missing ones, if not, they will keep the same,
   *     - if load data, `selectedIds` of the filtered data source and data sources which are derived from it will be the filtered result, if not, they will keep the same,
   *  - Selected record ids of data view info are subset of selected record ids of main data source info
   *    since some selected records of main data source may do not match filter of data view,
