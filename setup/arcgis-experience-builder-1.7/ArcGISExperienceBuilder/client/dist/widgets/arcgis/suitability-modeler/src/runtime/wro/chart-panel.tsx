...
import { Button, ButtonGroup, Select } from 'jimu-ui'
...
export class ChartPanel extends React.PureComponent<any, State> {
  ...
  computeHistograms = async (task): Promise<any> => {
    ...
    this.setState({ isWorking: true, showWarning: false })
    try {
      ...
      if (workingId === this.workingId) {
        this.setState({
          isWorking: false,
          showWarning: true,
          warning: nls('wro_validation_errorLoadingItem')
        })
      }
    }
  }
  ...
  render (): any {
    ...
    if (!canComputeHistograms) {
      ...
      if (activeTool === 'selectLayer') {
        if (layers && layers.length > 0) {
          ...
          selectNode = (
            <span className='widget-wro-chart-note'>{nls('wro_chart_noSubjectLayers')}</span>
          )
        }
      } else {
        // selectNode = (
    ...
    if (isWorking) {
      ...
      msgNode = <div className='widget-wro-chart-warning'>{this.state.warning}</div>
    }
