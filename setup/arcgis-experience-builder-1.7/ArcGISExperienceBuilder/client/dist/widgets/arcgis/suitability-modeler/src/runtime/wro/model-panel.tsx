...
import { AlertPopup, Button, ButtonGroup, Icon, NumericInput } from 'jimu-ui'
...
export class ModelPanel extends React.PureComponent<any, State> {
  ...
  exportModel = async (task: any): Promise<any> => {
    ...
    try {
      ...
      this.setState({ isExporting: false, alertMessage: msg })
    }
  }
  ...
  renderButtons (): any {
    ...
    if (showExport) {
      buttons.push(
        <Button
          key='export' disabled={!isValid || !hasMap || isRunning || isExporting}
    ...
    buttons.push(
      <Button
        key='run' type='primary' disabled={!isValid || !hasMap || isRunning}
