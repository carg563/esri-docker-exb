import {
  ...
  getAppStore, MutableStoreManager, ImmutableObject, dataSourceUtils, FeatureLayerDataSource
} from 'jimu-core'
...
export default class MapBase extends React.PureComponent<Props, State> {
  ...
  queryExtentFromQueryParams = (dataSourceId: string) => {
    ...
    return loadArcGISJSAPIModules([
      ...
      return layerObjectPromise.then(layerObject => {
        return layerObject.queryExtent(query).then(result => {
          ...
          if (result?.count === 1 && extentCenter) {
    ...
    if (mutableStateProps.zoomToFeatureActionValue) {
      if (mutableStateProps.zoomToFeatureActionValue.relatedWidgets &&
        ...
        if (zoomToFeatureValue.type === 'zoom-to-extent') {
          ...
          this.queryExtentFromQueryParams(zoomToFeatureValue.dataSourceId).then((result) => {
            let target
            if (result.count === 0) {
              return
    ...
    if (mutableStateProps.panToActionValue) {
      if (mutableStateProps.panToActionValue.relatedWidgets &&
        ...
        const relatedWidgets = mutableStateProps.panToActionValue.relatedWidgets
          ...
          : []
        this.queryExtentFromQueryParams(panToValue.dataSourceId).then((result) => {
