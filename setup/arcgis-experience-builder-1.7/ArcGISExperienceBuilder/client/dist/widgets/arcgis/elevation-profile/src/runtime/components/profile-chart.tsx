  ...
  */
  ...
  getDistanceOfAllSelectedFeatures = (): Promise<any[]> => {
    ...
    this.props.drawingLayer.graphics.forEach((selectedFeature) => {
      const selectedLayerId = selectedFeature.attributes.esriCTFeatureLayerId
      const config = this.getConfiguredSettingsForLine(selectedLayerId)
      //If distance settings in field get the distance value from field else get it as per mapProjection
      if (config?.distanceSettings?.field && config.distanceSettings.type === 'field') {
        getDistancePromiseArray.push(this.getDistanceAsPerConfiguredFieldAndUnits(config.distanceSettings, selectedFeature))
      } else {
