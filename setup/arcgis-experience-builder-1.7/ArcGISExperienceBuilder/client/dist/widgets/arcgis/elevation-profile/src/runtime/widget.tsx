  ...
  */
      ...
      if (combinedResults?.length > 0) {
        combinedResults.forEach((feature) => {
          if (feature?.geometry?.paths?.length > 0) {
            ...
            if (geometryEngine.intersects(endPointsGeometry[0], firstPoint) ||
              geometryEngine.intersects(endPointsGeometry[0], lastPoint) ||
              geometryEngine.intersects(endPointsGeometry[1], firstPoint) ||
              geometryEngine.intersects(endPointsGeometry[1], lastPoint)) {
              const polylineSymbol = {
                type: 'simple-line',
                color: [252, 252, 3, 0.8],
    ...
    return (
      <div tabIndex={-1} className={'h-100 w-100 d-flex align-items-center mainSection'}>
        <div tabIndex={-1} className={'adjust-cards'}>
          <Card tabIndex={0} aria-label={this.nls('selectLinesDesc')} button
          ...
          <Card tabIndex={0} aria-label={this.nls('drawProfileDesc')} button
  ...
  render () {
    ...
    if (this.props.config.useMapWidget) {
      ...
      return (
        <WidgetPlaceholder
          icon={epIcon.elevationIcon} widgetId={this.props.id}
