...
export default class Setting extends BaseWidgetSetting<AllWidgetSettingProps<IMConfig>, State> {
  ...
  updateConfigAsPerNewWebMap = () => {
    ...
    if (this.props.useMapWidgetIds && this.props.useMapWidgetIds.length > 0) {
      const mapViewGroups = this.mvManager.getJimuMapViewGroup(this.props.useMapWidgetIds[0])
      ...
      if (mapViewGroups && mapViewGroups.jimuMapViews) {
        for (const id in mapViewGroups.jimuMapViews) {
          if (mapViewGroups.jimuMapViews[id].dataSourceId) {
            dataSourcesToMatch.push(mapViewGroups.jimuMapViews[id].dataSourceId)
          } else {
  ...
  getAvailableDataSources = (useMapWidgetIds) => {
    ...
    if (mapViewGroups && mapViewGroups.jimuMapViews) {
      for (const id in mapViewGroups.jimuMapViews) {
        if (mapViewGroups.jimuMapViews[id].dataSourceId) {
          const availableGroundLayers = mapViewGroups.jimuMapViews[id].view.map.ground.layers
          ...
          if (availableGroundLayers.length > 0) { // Use map ground elevation layer
            const groundLayer: any = mapViewGroups.jimuMapViews[id].view.map.ground.layers
            elevationLayer = groundLayer?.items[0]?.url
          }
          this.groundData.push({
            dataSourceId: mapViewGroups.jimuMapViews[id].dataSourceId,
          ...
          setTimeout(() => {
            this.setState({
              ...
              this.getConfigForSelectedDs(mapViewGroups.jimuMapViews[id].dataSourceId)
            })
          }, 50)
          if (mapViewGroups.jimuMapViews[id].isActive || mapViewGroups.jimuMapViews[id].isActive === undefined) {
            setTimeout(() => {
              this.props.onSettingChange({
                id: this.props.id,
                config: this.props.config.set('activeDataSource', mapViewGroups.jimuMapViews[id].dataSourceId)
              })
            }, 50)
            this.setState({
      ...
      if (this.dataSourceOptions.length > 1) {
        ...
        if (mapViewGroups && mapViewGroups.jimuMapViews) {
          for (const id in mapViewGroups.jimuMapViews) {
            let isGroundLayer: boolean = false
            if (mapViewGroups.jimuMapViews[id].view.map.ground.layers.length > 0) {
              const groundLayer: any = mapViewGroups.jimuMapViews[id].view.map.ground.layers
              elevationLayer = groundLayer?.items[0]?.url
              isGroundLayer = true
            }
