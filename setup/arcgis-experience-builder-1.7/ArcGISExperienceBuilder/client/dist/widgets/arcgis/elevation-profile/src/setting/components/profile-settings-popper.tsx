...
import { Label, Select, Option, Tooltip, Icon, defaultMessages as jimuUIDefaultMessages } from 'jimu-ui'
...
import { elevationTypeOptions, elevationTypeOptionsWithoutZ, projectionOptions, getConfigIcon, unitOptions } from '../constants'
...
export default class ProfileSettingPopper extends React.PureComponent<Props, IState> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      distanceType: this.props.config.distanceSettings.type,
      elevationType: this.props.config.elevationSettings.type,
      elevationUnits: this.props.config.elevationSettings.unit,
      distanceUnits: this.props.config.distanceSettings.unit,
      distanceFields: this.props.config.distanceSettings.field === '' ? [] : [this.props.config.distanceSettings.field]
    }
  }
  ...
  onDistanceLengthTypeChange = (evt) => {
    this.setState({
      distanceType: evt.target.value
    }, () => {
      this.props.updateProfileSettings('distanceSettings', 'type', this.state.distanceType)
    })
  }

  onDistanceValueUnitChange = (evt) => {
    this.setState({
      distanceUnits: evt.target.value
    }, () => {
      this.props.updateProfileSettings('distanceSettings', 'unit', this.state.distanceUnits)
    })
  }

  onDistanceFieldSelect = (allSelectedFields: IMFieldSchema[]) => {
    if (allSelectedFields.length === 0) {
      this.setState({
        distanceFields: []
      })
      this.props.updateProfileSettings('distanceSettings', 'field', '')
    } else {
      this.setState({
        distanceFields: [allSelectedFields[0].jimuName]
      })
      this.props.updateProfileSettings('distanceSettings', 'field', allSelectedFields[0].jimuName)
    }
  }
    ...
    return <div style={{ height: '100%', width: '100%' }} css={getAdvanceSettingsStyle(this.props.theme)}>
      <SettingSection>
        ...
        <SettingRow className={'pt-3 ep-divider-top'}>
          ...
          <Tooltip role={'tooltip'} tabIndex={0} aria-label={this.nls('distanceSettingTooltip')}
            title={this.nls('distanceSettingTooltip')} showArrow placement='top'>
            <div className='ml-2 d-inline ep-tooltip'>
              <Icon size={14} icon={epConfigIcon.infoIcon} />
            </div>
          </Tooltip>
        ...
        <SettingRow>
          ...
          <Select menuRole={'menu'} aria-label={this.state.distanceType} className={'selectOption'} name={'distanceValueType'}
            size={'sm'} value={this.state.distanceType}
            onChange={this.onDistanceLengthTypeChange}>
        ...
        <SettingRow className={this.state.distanceType === 'map' ? 'hidden' : ''}>
          <Label tabIndex={0} aria-label={this.nls('sourceField')} className='flex-grow-1 ep-label'>
            <div className='flex-grow-1 text-break setting-text-level-3'>
              {this.nls('sourceField')}
            </div>
          </Label>
          <FieldSelector className={'fieldSelectorWidth'}
            dataSources={[this.props.selectedLayerDataSource.layer]}
            isSearchInputHidden={true}
            onChange={this.onDistanceFieldSelect.bind(this)}
            isDataSourceDropDownHidden={true}
            useDropdown={true}
            selectedFields={Immutable(this.state.distanceFields)}
            types={Immutable([JimuFieldType.Number])}
            isMultiple={false}
            noSelectionItem={this.defaultSelectedItem}
          />
        ...
        <SettingRow className={this.state.distanceType === 'map' ? 'hidden' : ''}>
          ...
          <Select menuRole={'menu'} aria-label={this.state.distanceUnits} className={'selectOption'} name={'distanceUnit'}
            size={'sm'} value={this.state.distanceUnits}
            onChange={this.onDistanceValueUnitChange}>
            {unitOptions.map((option, index) => {
              return <Option role={'option'} aria-label={option.value} key={index} value={option.value}>
                {this.nls(option.value)}</Option>
            })}
