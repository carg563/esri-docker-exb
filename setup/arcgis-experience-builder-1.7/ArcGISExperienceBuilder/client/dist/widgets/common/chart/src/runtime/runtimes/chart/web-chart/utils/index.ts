...
import { CategoryType, ColorMatches, ConfigFields } from '../../../../../config'
 ...
 * @param intl
 */
export const convertData = (records: DataRecord[], query: IMFeatureLayerQueryParams, colorMatches: ColorMatches, dataSourceId: string, intl: IntlShape) => {
 ...
 */
export const useRecordsToChartData = (records: DataRecord[], query: IMFeatureLayerQueryParams, colorMatches: ColorMatches, dataSourceId: string): WebChartDataItem[] => {
  const intl = useIntl()
  return React.useMemo(() => convertData(records, query, colorMatches, dataSourceId, intl), [records, query, colorMatches, dataSourceId, intl])
}
...
export const useSelectedDataItems = (
  ...
  React.useEffect(() => {
    ...
    if (categoryType === CategoryType.ByField) {
      ...
      selectedDataItems = selectedRecordIds?.length ? getSelectedData(selectedRecordIds, data) : undefined
    }
