...
import { Select, hooks, defaultMessages as jimuiDefaultMessage, Switch, LinearUnit, UnitTypes } from 'jimu-ui'
...
const units = [UnitTypes.PERCENTAGE]
...
export const PieSeries = (props: PieSeriesProps): React.ReactElement => {
  ...
  return (
    <div className='serial-series w-100'>
      ...
      {
        dataLabelVisible && <>
          <SettingRow label={translate('lableType')}>
            <Select
              size='sm'
              className='w-50'
              value={labelType}
          ...
          <SettingRow label={translate('lableOffset')}>
            <InputUnit
              className='w-50'
              size='sm'
              min={-100}
      ...
      <SettingRow label={translate('grouping')}>
        <div className='slice-grouping w-50 d-flex justify-content-between'>
          ...
          />
          <ThemeColorPicker className='flex-shrink-0 mr-1' value={sliceGroupingFill.color as any} onChange={handleSliceGroupingColorChange} />
        </div>
      </SettingRow>
