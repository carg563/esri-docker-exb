...
describe('src/runtime/serial/utils', () => {
  it('convertData', () => {
    ...
    let result = convertData(records, query, colorMatches, CategoryType.ByGroup, null)
    expect(result).toEqual([{ x: 'a!', v: 0 }, { x: 'b!', v: 1 }])

    data = [{ x: 'a', v: 0 }, { x: 'b', v: 1 }, { x: null, v: 2 }]
    records = data.map(makeDummyRecord)
    result = convertData(records, query, colorMatches, CategoryType.ByGroup, null)
    expect(result).toEqual([{ x: 'a!', v: 0 }, { x: 'b!', v: 1 }])
    ...
    records = data.map(makeDummyRecord)
    result = convertData(records, query, colorMatches, CategoryType.ByField, null)
