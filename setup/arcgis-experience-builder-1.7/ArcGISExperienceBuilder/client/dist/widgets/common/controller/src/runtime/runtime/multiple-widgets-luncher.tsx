 ...
 * @param forbidden
 */
export const useFigureOutLayouts = (mode: DisplayType, sizes: IMSizeMap, start: ControlPosition, spaceX: number, spaceY: number, forbidden?: Partial<DOMRectReadOnly>): ControlPosition[] => {
...
export const MultipleWidgetsLuncher = (props: MultipleWidgetsLuncherProps) => {
  const { mode, start, spaceX, spaceY, sizes: propSizes, widgets = DefaultWidgets, onClose, forbidden } = props
  ...
  return <React.Fragment>
    {Object.entries(widgets).map(([id, runtimeInfo], idx) => {
      ...
      const rendered = runtimeInfo.state !== undefined && isClassLoaded
      ...
      return <FloatingPanel
        ...
        defaultSize={size}
        className={classNames({ 'd-none': !opened }, 'multiple-widget-launcher')}
