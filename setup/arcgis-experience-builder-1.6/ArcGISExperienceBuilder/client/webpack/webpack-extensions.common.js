...
const translatedLocales = [
  'en', 'ar', 'bs', 'ca', 'cs', 'da', 'de', 'el', 'es', 'et', 'fi', 'fr', 'he', 'hr', 'hu', 'id', 'it', 'ja', 'ko', 'lt', 'lv',
...
function generateTemplates(templatesFolder){
  const infos = {} //the key is template type
  const templatesDistFolder = path.join(webpackCommon.outputPath, 'templates')
  let existedInfos = {};
  let existedInfoPath = path.join(templatesDistFolder, 'templates-info-existed.json');
  if(!fs.existsSync(templatesDistFolder)){
    fs.mkdirSync(templatesDistFolder, { recursive: true })
  }
  if(fs.existsSync(templatesDistFolder)){
    fsExtra.emptyDirSync(templatesDistFolder)
  }
  if(fs.existsSync(existedInfoPath)){
    existedInfos = JSON.parse(fs.readFileSync(existedInfoPath));
  }
  ...
  templateTyps.forEach(tt => {
    ...
    webpackCommon.visitFolder(ttFolder, (templateFolder, templateName) => {
      ...
      const templateDistFolder = path.join(templatesDistFolder, tt, templateName)
      if (!fs.existsSync(templateDistFolder)) {
        fs.mkdirSync(templateDistFolder, { recursive: true })
      }

      fsExtra.copySync(templateFolder, templateDistFolder)
  ...
  fs.writeFileSync(path.join(templatesDistFolder, 'templates-info.json'), JSON.stringify(infos, null, 2), 'utf-8')
  fsExtra.copySync(path.join(templatesFolder, 'translations'), path.join(templatesDistFolder, 'translations'))
}
