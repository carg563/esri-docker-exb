...
import { React, css, jsx, DataSource } from 'jimu-core'
  ...
  render () {
    ...
    return (
      <div className='feature-info-component d-flex align-items-center p-2' css={style}>
        {togglable && !showContent && (
          <Button className='p-0' type='tertiary' icon size='sm' onClick={this.expand}>
            <RightFilled size='s' autoFlip/>
          </Button>
        )}
        {togglable && showContent && (
          <Button className='p-0 expanded' type='tertiary' icon size='sm' onClick={this.collapse}>
            <DownFilled size='s'/>
          </Button>
        )}
