...
export async function getPopupTemplate (outputDS: FeatureLayerDataSource, queryItem: ImmutableObject<QueryItemType>) {
  ...
  if (resultFieldsType === FieldsType.SelectAttributes) {
    const fields = resultDisplayFields
    if (popupInfo) {
      const fieldInfos = []
...
export async function generateQueryParams (
  ...
  if (resultFieldsType === FieldsType.SelectAttributes) {
    const fields = combineFields(resultDisplayFields, resultTitleExpression, outputDS.getIdField())
    Object.assign(queryParams, {
      outFields: fields
    })
  } else {
...
export async function executeQuery (
  ...
  if (!originDs?.layer || (originDs as any).isSelectionView()) {
    ...
    records.forEach((record) => {
      (record as any).feature.sourceLayer = (layerObject as any).associatedLayer || layerObject
    })
  }
  const queryResult = {
    records,
