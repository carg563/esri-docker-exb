    ...
    return loadArcGISJSAPIModules(['esri/Graphic']).then(modules => {
      ...
      switch (message.type) {
        ...
        case MessageType.DataSourceFilterChange:
          ...
          const fillterChangeDS = DataSourceManager.getInstance().getDataSource(filterChangeMessage.dataSourceId)
          const filterChangeMainDS = fillterChangeDS?.getMainDataSource()
          // check dsId of mainDS currently, will support view in the future.
          if (!actionConfig.useDataSources.some(useDataSource => useDataSource?.mainDataSourceId === filterChangeMainDS.id)) {
            break
          }
