...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<IMConfig>, State> {
  ...
  render () {
    ...
    if (this.state.selectContent === 'itemurl') {
      content = <SettingRow><TextInput id='oic-itemurl' placeholder={this.nls('itemurl')}/*"Item url" i18n*/ onAcceptValue={this.getOICfromUrl} /> </SettingRow>
    } else {
      content = <><SettingRow>
        <Select placeholder={this.state.selectContent === 'content'
          ? this.nls('contents')
          : this.state.selectContent === 'group'
            ? this.nls('groups')
            : this.state.selectContent === 'orgGroups' ? this.nls('org') : 'Select group/folder'} /*i18n*/
    ...
    return <div className="widget-setting-orientedimagery">
      ...
      <SettingSection title={this.nls('chooseOIC')}>
