  ...
  return <div className="widget-setting-controller jimu-widget-setting" css={style}>
    ...
    <SettingSection title={translate('behavior')}>
      ...
      <SettingRow flow="wrap" label={translate('openStart')}>
        ...
        {
          onlyOpenOne && <Select value={openStart} size='sm' onChange={handleOpenStartSingleChange} className="w-100">
    ...
    <SettingSection title={<LabelTooltip label={translate('appearance')} tooltip={translate('appearanceTip')} />}>
      ...
      <SettingRow label={translate('tooltip')}>
        <Switch checked={showTooltip} onChange={(evt) => onSettingConfigChange(['appearance', 'card', 'showTooltip'], evt.target.checked)}></Switch>
      </SettingRow>
      <SettingRow flow="no-wrap" label={translate('iconSizeOverride')}>
        <Select size='sm' value={size} onChange={(e) => onSettingConfigChange(['appearance', 'card', 'avatar', 'size'], e.target.value)} className="w-50">
