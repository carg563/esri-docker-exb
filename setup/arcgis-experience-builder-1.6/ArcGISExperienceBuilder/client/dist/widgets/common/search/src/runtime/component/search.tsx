/** @jsx jsx */
import { React, css, ReactRedux, IMState, jsx, polished, Immutable, ImmutableArray, LinkResult, LinkTo, DataSourceStatus, QueriableDataSource, lodash, AppMode, RecordSetChangeType } from 'jimu-core'
...
const SearchInput = (props: SearchSettingProps) => {
  ...
  const { config, className, reference, id, datasourceConfig, isShowSearchInput, onShowSearchInputChange } = props
  ...
  */
  const publishRecordCreateAction = () => {
    const geocodeRecords = loadAllDsRecord(resultServiceListRef.current, resultMaxNumber, id, true)
  ...
  */
  const initGeocodeList = (configItem: NewDatasourceConfigItem) => {
    ...
    const { configId, hint, geocodeURL, outputDataSourceId, label, searchServiceType } = configItem
    const datasourceListItem = serviceList?.[configId] || {}
    const updateItem = {
      hint: hint,
  ...
  */
  const getPlaceholder = () => {
    ...
    servicePlaceholder = servicePlaceholder || multipleSearchPlaceholder
