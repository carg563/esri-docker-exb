/** @jsx jsx */
import { React, ReactRedux, css, IMState, jsx, UseDataSource, ImmutableArray, LinkType, Immutable, defaultMessages as jimuCoreDefaultMessage } from 'jimu-core'
import { SettingChangeFunction } from 'jimu-for-builder'
import { SettingSection, SettingRow, SettingCollapse, LinkSettingPopup, IMLinkParam, OpenTypes } from 'jimu-ui/advanced/setting-components'
import { IMConfig, SearchResultView, MAX_RESULT, MAX_SUGGESTION, MAX_RECENT_SEARCHES } from '../../config'
import { hooks, TextInput, Checkbox, Switch, Button, defaultMessages as jimuiDefaultMessage } from 'jimu-ui'
...
const SearchOptions = (props: SearchOptionsProps) => {
  ...
  `
  const linkSettingTrigger = React.useRef<HTMLButtonElement>(null)
  ...
  const [openLinkSetting, setOpenLinkSetting] = React.useState(false)
  ...
  const handHintAccept = value => {
    if (!value) {
      ...
      setNewHint(value?.trim())
      onConfigChange(['hint'], value?.trim())
    }
  }
        ...
        >
          ...
          {searchResultView === SearchResultView.OtherWidgets && <SettingRow className='d-block' flow='wrap' label={nls('redirectSearchResult')}>
            <Button
              ...
              type='primary'
              onClick={() => { setOpenLinkSetting(!openLinkSetting) }}
              ref={linkSettingTrigger}
            >
              <div className='w-100 px-2 text-truncate'>
                <LinkTiltedOutlined size={16} className='add-data-icon'/>
                {nls('setLink')}
              </div>
            </Button>
            {openLinkSetting && (
              <LinkSettingPopup
                showDialog={openLinkSetting}
                onSettingCancel={() => { setOpenLinkSetting(false) }}
                onSettingConfirm={onSettingLinkConfirm}
                linkParam={linkParam}
                useDataSources={useDataSources}
                widgetId={id}
                trigger={linkSettingTrigger?.current}
