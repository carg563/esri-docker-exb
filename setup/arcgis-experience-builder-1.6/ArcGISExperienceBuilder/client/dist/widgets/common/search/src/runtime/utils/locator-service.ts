...
export const loadGeocodeRecodsAndUpdateOutputDs = (option: updateOutputDsRecordsOptions) => {
  const { address, maxResultNumber, searchResultView, geocodeItem } = option
  const { outputDataSourceId } = geocodeItem
  const outputDs = getDatasource(outputDataSourceId)
  return addressToLocations(geocodeItem, address, maxResultNumber).then(response => {
    return loadArcGISJSAPIModules(['esri/layers/FeatureLayer', 'esri/Graphic']).then(modules => {
      const [FeatureLayer, Graphic] = modules
      let newResponse = response?.filter(res => res?.address)
      newResponse = searchResultView === SearchResultView.ResultPanel ? newResponse?.splice(0, maxResultNumber) : newResponse
      let extent
...
*/
export const addressToLocations = (geocodeItem: GeocodeListItem, address: string, maxResultNumber: number): Promise<__esri.AddressCandidate[]> => {
