...
export const modifyGroupByField = (uniqueQuery: IMFeatureLayerQueryParams, splitByField: string, x: string): IMFeatureLayerQueryParams => {
  if (uniqueQuery.groupByFieldsForStatistics !== undefined) {
    if (!uniqueQuery.groupByFieldsForStatistics.includes(splitByField)) { uniqueQuery = uniqueQuery.set('groupByFieldsForStatistics', uniqueQuery.groupByFieldsForStatistics.concat(splitByField)) }
  } else {
    if (x === splitByField) {
