...
import { getSeriesType, getSplitByField, getSplitByValue } from 'jimu-ui/advanced/chart'
import { CategoryType, WebChartBarChartSeries, WebChartLineChartSeries, WebChartSeries } from '../../../../../config'
...
import { getFillSymbol, getLineSymbol, getSeriesFillColor } from '../../../../../utils/default'
...
export const createByFeatureSeries = ({ categoryField, numericFields, orderByFields, pageSize }, firstSerie, templateSeries, dataSourceId) => {
  const series = numericFields.map((numericField, index) => {
    ...
    const template = getByFeatureNewSerieTemplate(firstSerie, templateSeries[index], index) as any
    let serie = template.set('x', categoryField).set('y', y).set('name', name).set('id', y)
...
export const createByFieldSeries = ({ name, numericFields, statisticType, orderByFields }, templateSeries: ImmutableArray<WebChartSeries>) => {
  ...
  const template = templateSeries[0] as any
