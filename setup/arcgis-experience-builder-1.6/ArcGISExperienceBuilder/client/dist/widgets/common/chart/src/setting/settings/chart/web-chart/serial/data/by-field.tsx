import { React, ImmutableArray, UseDataSource, StatisticType, DataSourceSchema, Immutable } from 'jimu-core'
...
export const ByFieldData = (props: ByFieldDataProps): React.ReactElement => {
  ...
  const handleStatisticTypeChange = (evt: React.MouseEvent<HTMLSelectElement>): void => {
    ...
    const series = propSeries.map((serie) => {
      return Immutable.setIn(serie, ['query', 'outStatistics'], serie?.query?.outStatistics.map((outStatistic) => {
  ...
  const handleOrderChanged = (value: string): void => {
    ...
    const series = propSeries.map((serie) => {
      return Immutable.setIn(serie, ['query', 'orderByFields'], [orderByFields])
    }) as unknown as ImmutableArray<WebChartSeries>
    onChange?.(series)
  }
