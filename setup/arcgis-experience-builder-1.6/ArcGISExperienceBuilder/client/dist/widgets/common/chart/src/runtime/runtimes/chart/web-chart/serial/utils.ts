 ...
 * @param intl
 */
export const mapRecordsToFormattedData = (records: DataRecord[], x: string, categoryType: CategoryType, intl: IntlShape): [any[], Map<string, any>] => {
  const chartData = []
...
export const useParseData = (
  ...
  const [data, xFormattedValueMap] = React.useMemo(() => mapRecordsToFormattedData(records, categoryField, categoryType, intl), [categoryType, categoryField, intl, records])
...
export const useWebChartVersion = (webChart: ImmutableObject<IWebChart>, numberOfSeries: number): number => {
  ...
  hooks.useUpdateEffect(() => {
    ...
    const ret = (series?.map(serie => {
      return (serie as any).without('query')
 ...
 * @param attributes
 */
export const getRecords = (records: DataRecord[], attributes: Array<{ [key: string]: string }>): DataRecord[] => {
 ...
 * @param attrs
 */
export const getRecordIds = (records: DataRecord[], attrs: Array<{ [key: string]: string }>): string[] => {
 ...
 */
export const convertChartDataToRecordAttributes = (data: WebChartDataItem[], splitByField: string): Array<{ [key: string]: string }> => {
  if (!splitByField) return data as Array<{ [key: string]: string }>
 ...
 */
export const convertRecordAttributesToChartData = (attributes: Array<{ [key: string]: string }>, splitByField: string): WebChartDataItem[] => {
...
export const getPartialRecords = (records: Array<{ [x: string]: any }>, fields: string[] | ImmutableArray<string>) => {
 ...
 * @returns
 */
export const getSelectedData = (selectedIds: string[], data: WebChartDataItem[], fields: string[]): WebChartDataItem[] => {
  ...
  const selectedData = getPartialRecords(selectedDataItems, fields)
  return selectedData
}
/**
 ...
 * @param categoryField
 */
export const getSelectedDataItems = (dataSource: DataSource, data: WebChartDataItem[], fields: string[], splitByField?: string): WebChartDataItem[] => {
  if (!dataSource || !data?.length) return

  let selectedDataItems
  if (!splitByField) {
    const selectedRecordIds = dataSource.getSelectedRecordIds()
    selectedDataItems = getSelectedData(selectedRecordIds, data, fields)
  } else {
