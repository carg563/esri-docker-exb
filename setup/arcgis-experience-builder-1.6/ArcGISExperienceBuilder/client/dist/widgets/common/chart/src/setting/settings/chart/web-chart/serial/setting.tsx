import { React, ImmutableArray, UseDataSource, ImmutableObject, QueriableDataSource, DataSourceSchema, Immutable } from 'jimu-core'
...
export const SerialSetting = (props: ChartSettingProps): React.ReactElement => {
  ...
  const handleRotatedChange = (rotated: boolean): void => {
    const series = webChart?.series.map(serie => Immutable.set(serie, 'rotated', rotated))
    onWebChartChange?.(webChart.set('series', series))
  }
