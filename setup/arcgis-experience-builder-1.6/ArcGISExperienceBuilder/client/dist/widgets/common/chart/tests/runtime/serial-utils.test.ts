...
describe('src/runtime/serial/utils', () => {
  ...
  it('mapRecordsToChartData', () => {
    ...
    let result = mapRecordsToFormattedData(records, x, CategoryType.ByGroup, null)

    expect(result[0]).toEqual([{ x: 'a!', v: 0 }, { x: 'b!', v: 1 }])
    let map = new Map()
    map.set('x-a!', 'a')
    map.set('x-b!', 'b')
    ...
    result = mapRecordsToFormattedData(records, x, CategoryType.ByGroup, null)
    expect(result[0]).toEqual([{ x: 'a!', v: 0 }, { x: 'b!', v: 1 }])
    map = new Map()
    map.set('x-a!', 'a')
    map.set('x-b!', 'b')
    ...
    data = [{ x: 'a', v: 0 }, { x: 'b', v: 1 }, { x: null, v: 2 }]
    records = data.map(makeDummyRecord)
    result = mapRecordsToFormattedData(records, x, CategoryType.ByField, null)
