...
> {
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    const { editFeatures, dataSources } = this.state
    ...
    if (settingChange) {
      if (editMode === EditModeType.Geometry) {
        ...
        if (edit?.formTemplate && layersConfig.length !== 0) {
          const elements = this.constructFormElements()
          if (edit?.formTemplate) edit.formTemplate.elements = elements
  ...
  createEditor = (mapView?) => {
    ...
    const { selfSnappingCheck, featureSnappingCheck } = this.state
    ...
    layersConfig.forEach(async layerConfig => {
      const { groupedFields, addRecords, deleteRecords, updateGeometries, layerId, id: configDsId } = layerConfig
      const mapLayers = jimuMapView.view.map.layers
      let useMapLayer = mapLayers.find(layer => layer.id === layerId)
      if (!useMapLayer) {
        mapLayers.forEach(layer => {
          const subLayers = (layer as any)?.layers
          if (subLayers && (configDsId.includes(layer.id) || configDsId.includes(layerId))) {
            useMapLayer = subLayers.find(layer => (layer.layerId?.toString() === layerId || layer.id === layerId))
          }
        })
      }
      // fetch to confirm whether it's a public source
      const accessible = await this.getDsAccessibleInfo((useMapLayer as any)?.url)
      // use exb privilege instead of api's supportsUpdateByOthers
      const privilegeEditable = await this.getPrivilege()
      const editable = accessible || privilegeEditable
      editLayerInfos.push({
        layer: useMapLayer,
        fieldConfig: this.constructFieldConfig(groupedFields)?.asMutable({ deep: true }),
