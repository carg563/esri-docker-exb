...
> {
  ...
  importAll = () => {
    ...
    const useMapWidget = useMapWidgetIds?.[0]
    if (!useMapWidget) return
    const appConfig = getAppStore().getState().appStateInBuilder.appConfig
    const mapUseDataSources = appConfig.widgets[useMapWidget]?.useDataSources
    const mapUseDataSourcesIds = mapUseDataSources.map(item => item.dataSourceId)
    const useDataSources = []
    mapUseDataSourcesIds.forEach(dsId => {
      const currentDs = this.dsManager.getDataSource(dsId)
        ...
        <div className='widget-setting-edit'>
          ...
          <SettingSection className={`p-0 m-0 ${itemsLength > 0 ? '' : 'border-0'}`}>
            <div ref={this.sidePopperTrigger}>
                  ...
                  <div className='layerlist-tools w-100'>
                    <div className='w-100 pb-1'>
                      <div className='layerlist-tools-item radio'>
                        <Radio
                          id='edit-attr' style={{ cursor: 'pointer' }}
                          name='edit-attr' onChange={() => this.onEditModeChange(EditModeType.Attribute)}
                          checked={isAttrMode}
                        />
                        <Label style={{ cursor: 'pointer' }} for='edit-attr' className='ml-1'>
                    ...
                    <div className='w-100'>
                      <div className='layerlist-tools-item radio'>
                        <Radio
                          ...
                          name='map-view' onChange={() => this.onEditModeChange(EditModeType.Geometry)}
                          checked={isGeoMode}
                        />
              ...
              {isGeoMode && useMapWidgetIds?.length > 0 && itemsLength > 0 &&
                <SettingSection className='pt-0 border-0'>
                  <SettingRow label={this.formatMessage('snappingSettings')} className='capability-header' />
                  <div className='capability-item' key={'selfSnapping'}>
                  ...
                  <div className='capability-item' key={'featureSnapping'}>
