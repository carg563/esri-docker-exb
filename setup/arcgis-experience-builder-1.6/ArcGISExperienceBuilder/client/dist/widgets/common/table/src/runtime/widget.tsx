...
> {
  ...
  dsAsyncSelectTable = (dataSource, selectedItems, versionChangeClear?) => {
    const { id } = this.props
  ...
  createTable = (newDataSource?, versionChange?) => {
    ...
    const getLayerAndNewTable = () => {
      ...
      const dataSourceUsed = isSceneLayer ? (dataSource as any).associatedDataSource : dataSource
      dataSourceUsed &&
        this.getFeatureLayer(dataSourceUsed, dataRecords).then(async layer => {
          ...
          if (versionChange) {
            const versionChangeReselect = () => {
              const selectedItems: any[] = dataSource.getSelectedRecords()
          ...
          if (curLayer.enableSelect) {
            const rowClickFn = ({ context, native }) => {
              ...
              this.timerFn = setTimeout(() => {
                ...
                this.dsAsyncSelectTable(dataSource, selectedItems)
              }, 200)
            }
