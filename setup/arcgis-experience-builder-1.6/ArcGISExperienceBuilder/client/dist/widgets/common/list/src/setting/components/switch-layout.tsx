  ...
  const renderRemindModel = () => {
    return (
      <Modal isOpen={isOpen} toggle={handleToggle} centered>
        ...
        <ModalFooter>
          <Button onClick={() => { setIsOpen(false) }}>{nls('cancel')}</Button>
  ...
  return (
    <div css={STYLE}>
      <SettingRow label={nls('layout')} flow='wrap'>
        <Radio checked={config?.cardConfigs?.[status]?.listLayout === ListLayout.AUTO} onChange={() => { handleLayoutChange(ListLayout.AUTO) }} className='mr-2'/> {nls('auto')}
      </SettingRow>
      <SettingRow >
        <Radio checked={config?.cardConfigs?.[status]?.listLayout === ListLayout.CUSTOM || !config?.cardConfigs?.[status]?.listLayout} onChange={() => { handleLayoutChange(ListLayout.CUSTOM) }} className='mr-2'/> {nls('custom')}
      </SettingRow>
      {renderRemindModel()}
    </div>
  )
